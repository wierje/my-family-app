...
describe('Directive: oauthButtons', function() {
  ...
  it('should contain anchor buttons', function() {
    ...
    $(element[0]).find('a.btn.btn-social').length.should.be.at.least(1);
  });
  ...
  it('should bind scope.classes to class names on the anchor buttons', function() {
    ...
    elementScope.$digest();
    $(element[0]).find('a.btn.btn-social.testClass1.testClass2').length.should.be.at.least(1);

    // Remove classes
    elementScope.classes = '';
    elementScope.$digest();
    $(element[0]).find('a.btn.btn-social.testClass1.testClass2').length.should.equal(0);
  });
});
